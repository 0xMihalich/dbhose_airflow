...
.. py:decorator:: NativeDumper.multiquery
   ...
   и одной операции с данными в Native формате.
...
* ``to_reader()`` - для получения объекта ``NativeReader``
...
2. **Первая часть** - все запросы до Native операции
3. **Вторая часть** - все запросы после Native операции
4. **Native операция** - определяется по ключевым словам (``SELECT``, ``INSERT`` и т.д.)
...
- :doc:`read_dump` - Чтение данных с поддержкой мультизапросов
- :doc:`write_between` - Перенос данных с поддержкой мультизапросов
- :doc:`to_reader` - Чтение данных из ClickHouse в виде объекта NativeReader
